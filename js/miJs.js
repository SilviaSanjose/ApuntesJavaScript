...
for (var i in numeros) {
  //for in debe llevar el nº del elemento. Se usa más para objetos
...
for (var i of numeros) {
  //for of muestra directamente el elemento. Para arrays
  console.log(i);
}
...
(() => {
  ...
  persona[SALUDO] = () => {
    console.log("Hola Symbol");
  }; //añado función Symbol al objeto
  persona[SALUDO]();
})();
...
function cuadradoCallback(value, callback) {
  //función recibe como parámetros un valor y una función llamada callback
  setTimeout(() => {
...
cuadradoCallback(2, (valor, result) => {
  ...
  cuadradoCallback(3, (valor, result) => {
    //se vuelve a llamar a la función... que no se ejecuta hasta que no termine la anterior, se puede producir Callback Hell*
    console.log(`Callback: ${valor}, ${result}`);
  });
});
...
function cuadradoPromesas(value) {
  ...
  return new Promise((resolve, reject) => {
    setTimeout(() => {
      resolve({
        //creamos objeto con value y result como claves
        value,
        result: value * value,
      });
...
cuadradoPromesas(0)
  .then((obj) => {
    //recibe el resolve, en este caso nuestro objeto
...
(function () {
  //añado setTimeout en vez de llamada directamente a asincrona() para que se muestre después de las promesas en consola
  setTimeout(() => {
    asincrona();
  }, 4000);
  setTimeout(() => {
